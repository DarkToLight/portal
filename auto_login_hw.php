<?php
	...
	$check = str_replace(":","",$usermac); 	
		
		$q = "select id from radcheck where username='".$check."'";
        ...
        if(mysqli_num_rows($rsSelect) ==1){
			...
			$q = "insert into radcheck(username,attribute,op,value,type) value('".$check."','User-Password',':=','".$check."',4)";
            $rs = mysqli_query($link,$q);
            if(!$rs){
