<?php
	...
	$check = rand(100000,999999);	
		...
		$q = "select id from radcheck where username='".$websuserip."'";
        ...
        if(mysqli_num_rows($rsSelect) ==1){
			...
			$q = "insert into radcheck(username,attribute,op,value,type) value('".$websuserip."','User-Password',':=','".$check."',4)";
            $rs = mysqli_query($link,$q);
            if(!$rs){
		...
		$curl = curl_init(); 
		$url = "http://10.20.0.6:8000/login/wechat";
